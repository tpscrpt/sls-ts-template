...
require("yargs")
    ...
    .command("new-table <name>", 'create a new table', (yargs) => {
        ...
        yargs.option("getItem", {
            type: "array",
            describe: "creates a getItem policy for this table, optionally applying it to a list of function names",
            alias: "g",
            default: []
        })
        yargs.option("putItem", {
            type: "array",
            describe: "creates a putItem policy for this table, optionally applying it to a list of function names",
            alias: "p",
            default: []
        })
        yargs.option("updateItem", {
            type: "array",
            describe: "creates a updateItem policy for this table, optionally applying it to a list of function names",
            alias: "u",
        })
        yargs.option("deleteItem", {
            type: "array",
            describe: "creates a deleteItem policy for this table, optionally applying it to a list of function names",
            alias: "d",
        })
        yargs.option("query", {
            type: "array",
            describe: "creates a query policy for this table, optionally applying it to a list of function names",
            alias: "q"
        })
        yargs.option("scan", {
            type: "array",
            describe: "creates a scan policy for this table, optionally applying it to a list of function names",
            aslas: "s"
        })
...
async function newFunction(name) {
    // TODO: refactor to not use these prototype functions, use replaceAll like in newTable
    // Local functions used on Strings
    ...
    String.prototype.capitalizeFirst = function(name) {
        const str = this.valueOf()
        return str.replace(new RegExp('{{name.capitalizeFirst}}', 'g'), _capitalizeFirst(name))
    }
        ...
        new Promise((resolve) => {
            ...
            fs.writeFileSync(functionIndexTsPath,
                fs.readFileSync(functionIndexTsTemplatePath)
                    .toString().replaceAll('{{name.capitalizeFirst}}', _capitalizeFirst(name))
            )
            resolve()
        }),
    ])
